...
class Camionetas():
     def _init_(self,color,marca,modelo,velocidad,caballaje,plazas,traccion,cerrada):
      super()._init_(color,marca,modelo,velocidad,caballaje,plazas,traccion,cerrada)
...
def getNum_Pasajeros(self):
     return self.__num_pasajeros

def setNum_Pasajeros(self,num_pasajeros):
...
def setNum_pasajeros(self,num_pasajeros):
  ...
  #print("objeto 3")
camion1=Camiones("negro","Bina","2020",100,300,12,8,2500)
camion1.getinfo()

def getinfo(self):
   print(f"""marca {self.getmarca()}{self.getcolor()} numero de plazas:
         {self.getplazas()}
      modelo: {self.getmodelo()} con una velocidad de: {self.getvelocidad()} km por hora
      con una potencia de {self.getcaballaje()}""")
